...
namespace CMS2.CentralWeb.Areas.Admin.Controllers
{
    ...
    {
        private PaymentTypeBL service = new PaymentTypeBL();
        ...
        public ActionResult Add(PaymentType model)
        {
            ...
            {
                model.PaymentTypeId = Guid.NewGuid();
        ...
        public ActionResult Edit(PaymentType model)
        {
            if (ModelState.IsValid)
            {
                var _model = service.GetById(model.PaymentTypeId);
                _model.PaymentTypeName = model.PaymentTypeName;
