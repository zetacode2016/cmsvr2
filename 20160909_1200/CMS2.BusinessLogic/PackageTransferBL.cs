        ...
        public List<PackageTransfer> GetByDateTransferTypeBcoId(DateTime date, string transferType, Guid bcoId)
        {
            ...
            switch (transferType)
            {
                case TransferTypeConstant.BsoToBco:
                    ...
                    var bsos = bsoService.FilterBy(x => x.RevenueUnitType.RevenueUnitTypeName.Equals(RevenueUnitTypes.BSO) && x.Cluster.BranchCorpOffice.BranchCorpOfficeId == bcoId).Select(x=>x.RevenueUnitId).ToList();
                    models =
                        models.Where(
