...
namespace CMS2.BusinessLogic
{
    public class TrackingBL
    {
        ...
        public List<BundleViewModel> GetBundlesByDateOriginBCO(DateTime date, string originBCO)
        {
            ...
            var employeeIds = employeePositionService.GetByDateBcoName(date, originBCO).Select(x=>x.EmployeeId).ToList();
            if (employeeIds != null)
            {
        ...
        {
            List<string> usernames = new List<string>();
            var employeeIds = employeePositionService.GetByDateBcoName(date, bco).Select(x => x.EmployeeId).ToList();
            if (employeeIds != null)
            {
                var users = userService.GetAllUsers();
                foreach (var item in employeeIds)
        ...
        public List<DeliveryStatusViewModel> GetShippingStatusByAwb(string awb)
        {
            ...
            if (shipments != null && shipments.Count > 0)
            {
                ...
                mapping = employeePositionService.GetByEmployeeDate(user.EmployeeId, shipment.DateAccepted);
                if (mapping != null)
                ...
                if (acceptancePackages != null && acceptancePackages.Count > 0)
                {
                    ...
                    mapping = employeePositionService.GetByEmployeeDate(user.EmployeeId, acceptance.dDateTime ?? DateTime.Now);
                    if (mapping != null)
                        city = mapping.AssignedLocation.City.CityName;
                    ...
                    if (transmittals != null && transmittals.Count > 0)
                    {
                        ...
                        mapping = employeePositionService.GetByEmployeeDate(user.EmployeeId, transmittal.dDateTime ?? new DateTime());
                        if (mapping != null)
                            city = mapping.AssignedLocation.City.CityName;
                        ...
                        if (inbounds != null && inbounds.Count > 0)
                        {
                            ...
                            mapping = employeePositionService.GetByEmployeeDate(user.EmployeeId, inbound.dDateTime ?? new DateTime());
                            if (mapping != null)
                                city = mapping.AssignedLocation.City.CityName;
                            ...
                            if (acceptancePackages != null && acceptancePackages.Count > 0)
                            {
                                ...
                                if (distributions != null || distributions.Count > 0)
                                {
                                    ...
                                    mapping = employeePositionService.GetByEmployeeDate(user.EmployeeId , distribution.dDateTime ?? new DateTime());
                                    if (mapping != null)
                                        city = mapping.AssignedLocation.City.CityName;
                                    ...
                                    if (deliveries != null && deliveries.Count > 0)
                                    {
                                        ...
                                        mapping = employeePositionService.GetByEmployeeDateCityName(user.EmployeeId, delivery.DateDelivered, delivery.Shipment.DestinationCity.CityName);
                                        if (mapping != null)
                                            city = mapping.AssignedLocation.City.CityName;
